  ...
  `)}function _(){h(`
    <section class="tags-page">
      ...
      <div class="tag-filter-list" data-tag-filters>
        ...
        ${[...new Set(Object.values(a).flatMap(e=>e.tags||[]))].sort((e,t)=>e.localeCompare(t,`ru`)).map(e=>`<button class="tag-filter" type="button" data-tag-filter="${e}">${e}</button>`).join(``)}
      </div>
      <div class="tag-results" data-tag-results></div>
    </section>
  `,`tags`);let e=document.querySelector(`[data-tag-results]`),t=document.querySelectorAll(`[data-tag-filter]`),n=(t=``)=>{let n=Object.entries(a).filter(([,e])=>!t||(e.tags||[]).includes(t));e.innerHTML=n.length?n.map(([e,t])=>`<a class="tag-result reveal" href="#/article/${e}"><span class="card-year">${t.year}</span><h2>${t.title}</h2><p>${t.subtitle}</p><div class="article-tags">${(t.tags||[]).map(e=>`<span class="tag-chip">${e}</span>`).join(``)}</div><span class="arrow">↗</span></a>`).join(``):`<p class="tag-empty">Статей с таким тегом пока нет.</p>`};t.forEach(e=>{e.addEventListener(`click`,()=>{t.forEach(t=>t.classList.toggle(`active`,t===e)),n(e.dataset.tagFilter)})}),n()}async function v(){if(!i||i.role!==`admin`){f(`login`);return}try{let[e,t,n]=await Promise.all([c(`/api/admin/articles`,{method:`GET`}),c(`/api/admin/comments`,{method:`GET`}),c(`/api/admin/tags`,{method:`GET`})]);h(`
...
`)}</textarea>
                    <textarea name="fact" placeholder="Заметка из архива" required>${e.fact||``}</textarea>
...
`).map(e=>e.trim()).filter(Boolean);try{await c(`/api/admin/articles`,{method:`POST`,body:JSON.stringify({...t,body:n})}),r.reset(),v()}catch(e){alert(e.message)}});let i=document.querySelector(`[data-admin-tag-form]`);i?.addEventListener(`submit`,async e=>{e.preventDefault();let t=Object.fromEntries(new FormData(i));try{await c(`/api/admin/tags`,{method:`POST`,body:JSON.stringify({name:t.tag})}),i.reset(),v()}catch(e){alert(e.message)}}),document.querySelectorAll(`[data-article-id]`).forEach(e=>{e.addEventListener(`submit`,async t=>{t.preventDefault();let n=new FormData(e).get(`tag`);try{await c(`/api/admin/articles/${e.dataset.articleId}/tags`,{method:`POST`,body:JSON.stringify({tag:n})}),v()}catch(e){alert(e.message)}})}),document.querySelectorAll(`[data-edit-article-id]`).forEach(e=>{e.addEventListener(`submit`,async t=>{t.preventDefault();let n=Object.fromEntries(new FormData(e)),r=n.body.split(`
`).map(e=>e.trim()).filter(Boolean);try{await c(`/api/admin/articles/${e.dataset.editArticleId}`,{method:`PUT`,body:JSON.stringify({title:n.title,body:r,fact:n.fact})}),v()}catch(e){alert(e.message)}})}),document.querySelectorAll(`[data-delete-article-tag]`).forEach(e=>{e.addEventListener(`click`,async()=>{try{await c(`/api/admin/articles/${e.dataset.deleteArticleTag}/tags/${e.dataset.tagId}`,{method:`DELETE`}),v()}catch(e){alert(e.message)}})}),document.querySelectorAll(`[data-delete-tag]`).forEach(e=>{e.addEventListener(`click`,async()=>{try{await c(`/api/admin/tags/${e.dataset.deleteTag}`,{method:`DELETE`}),v()}catch(e){alert(e.message)}})}),document.querySelectorAll(`[data-delete-comment]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.deleteComment;try{await c(`/api/admin/comments/${t}`,{method:`DELETE`}),v()}catch(e){alert(e.message)}})})}catch(e){alert(e.message)}}async function y(e){try{let t=await fetch(`/api/comments/${e}`,{credentials:`same-origin`}),n=await t.json();if(!t.ok)throw Error(n.error||`Не удалось загрузить комментарии.`);return n.comments||[]}catch{return[]}}async function b(){let e=document.querySelector(`[data-comments-root]`);if(!e)return;let t=e.dataset.articleKey,n=await y(t),r=!!i;e.innerHTML=`
  ...
  `,e),b()}async function S(){await u(!0);let e=window.location.hash.replace(`#`,``)||`/`;if(e===`/admin`){await v(),window.scrollTo(0,0);return}if(e===`/tags`){_(),window.scrollTo(0,0);return}let t=e.match(/^\/article\/([^/]+)$/);t?x(t[1]):g(),window.scrollTo(0,0)}window.addEventListener(`hashchange`,S),S();